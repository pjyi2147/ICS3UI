...
struct maze {
	char map[12][12] = { {'#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#'},
                        ...
                        {'#', '.', '.', '.', '.', '.', '.', '#', '.', '.', '.', '#'},
                        {'#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#'} };
	int dir = 1;				// 0 right, 1 down, 2 left, 3 up (modulo) ;; counterclockwise
};
...
void mazeTraverse(maze &maze, int &x, int &y, int *x1, int *x2) {
	maze.map[y][x] = 'x';
	printMap(maze);
	...
	for (int i = 0; i < 12; i++) {                  // row
		for (int j = 0; j < 12; j++) {              // column
			if (i == 0 || i == 11) {
				...
				if (rand() % 5) {                       // to have some more dots than hashtag
					maze.map[i][j] = '.';
				}
				else {									// walls
					maze.map[i][j] = '#';
...
int main() {
	...
	int*x1 = &x;
	int*y1 = &y;
	cout << "Searching route from " << "(" << x << ", " << y << ")" << endl << endl;
